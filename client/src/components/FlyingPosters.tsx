// client/src/components/FlyingPosters.tsx
'use client';
...
function AutoBind(self: any, { include, exclude }: any = {}) {
  const getAllProperties = (object: any): Set<[any, PropertyKey]> => {
    const properties = new Set<[any, PropertyKey]>();
    do {
      for (const key of Reflect.ownKeys(object) as PropertyKey[]) {
  ...
  for (const [object, key] of getAllProperties(self.constructor.prototype)) {
    if (key === 'constructor' || !filter(key as string)) continue;
...
export default function FlyingPosters({
  ...
  return (
    <div ref={containerRef} className={`w-full h-full overflow-hidden relative z-20 ${className}`} {...props}>
